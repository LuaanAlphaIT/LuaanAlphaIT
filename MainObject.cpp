...
MainObject::MainObject()
{
	x_val_ = 0;
	y_val_ = 0;
	ifstream pos_file("Map/Pos_.txt");
	pos_file >> x_pos_;
	pos_file >> y_pos_;
	pos_file.close();
	frame_ = 0;
...
void MainObject::Show(SDL_Renderer* des, int cam_y)
{
	...
	else if (input_type_.UP_ == 1 || input_type_.v_ == 1 || input_type_.x_ == 1)
	{
		bool ok = 0;
		if (input_type_.UP_ == 1)
		{
			LoadImg("Img/UP.png", des);
			ok = 1;
		}
		else if (input_type_.v_ == 1)
		{
			LoadImg("Img/v.png", des);
			ok = 1;
		}
		else if (input_type_.x_ == 1)
		{
			LoadImg("Img/x.png", des);
			ok = 1;
		}
		frame_++;
		if (frame_ >= 5)
